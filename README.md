...
- 📡 **METLAB 연구 체험 프로그램 대회 (2024.07 ~ 2024.09)**   
  ESP32-CAM CSI + 영상 데이터 융합 → 악천후 인식 정확도 개선 / **총장상 수상**  
...
- **내용:** 운전자의 급발진 및 페달 오조작 상황을 조기에 감지하는 멀티모달 AI 기반 시스템  
  - **입력:**  
    - 얼굴 표정(Facial Emotion Recognition, IMX500) → 놀람·분노 감정 탐지  
...
- **기술:** Raspberry Pi 5, TensorFlow Lite (FER 모델), openSMILE (Audio Feature Extraction), Python 멀티스레딩, OBD-II 시리얼 통신
- **성과:**  
...
###  🚙 RC카 기반 프로젝트 (2025.05 ~ 2025.06)
- **역할:** 시스템 통합 개발
- **내용:** RC카를 이용한 라인트레이싱 & 원격 제어  
  - Picamera2 + Flask → 실시간 웹 스트리밍  
  - Servo/ESC 제어 + PID 알고리즘 기반 라인트레이싱  
  - Radiolink AT9 조종기 & R9DS 수신기 신호 수신
- **기술:** Raspberry Pi 5, Arduino Nano, Python Flask, PID 제어
- **성과:** 자율주행 & 수동 전환, 긴급사출 기능 구현
...
### 🈶 한글 문자 인식 인공신경망 (2024.10 ~ 2024.12)
- **내용:** 64×64 비트맵 기반의 한글 문자 이미지 데이터셋 구축 및 분류 모델 설계  
  - 여러 글자를 대상으로 **폰트/위치 변형을 가미한 데이터 100장씩 생성**  
  - 총 **115 클래스(one-hot 인코딩)** 라벨 구조로 확장하여 학습 가능하도록 데이터셋 구성  
  - **순전파(Forward) & 역전파(Backpropagation)** 알고리즘을 Python과 C 언어로 직접 구현
- **기술:** Python, C 언어, Custom Neural Network Implementation, 데이터 전처리 & 증강
- **성과:** 단순 코드 실습을 넘어, **직접 데이터셋 생성 → 모델 학습/평가 → 한계 분석**까지 수행하여  
  인공지능 모델 개발 사이클(AI pipeline)을 경험
...
- **내용:** ESP32-CAM을 활용하여 영상 + Wi-Fi CSI(Channel State Information) 데이터 수집 → 멀티모달 인식  
  - 수집한 데이터를 전처리(결측치 제거, Normalization) 후 **랜덤포레스트(Random Forest) 모델을 적용하여 AI 기반 인식 성능을 평가**
- **기술:** ESP-IDF, ESP-CSI 라이브러리, Random Forest (AI 모델), 데이터 전처리
...
## 📌 Web/App Development
### 🌐 INU 정보기술대 전용 SNS – ESC 소모임 (2024.09 ~ 2025.01)
- **역할:** 프론트엔드 팀장  
...
## 📌 Organizations
- [Pedal Misoperation Detection System](https://github.com/brake-down)
- [양봉 프로젝트](https://github.com/WCL-INU/BEE_COUNT_YOLO5)
- [핸들 압력 패턴 기반 반자율주행 보행기](https://github.com/Capstone-A-Sharp)
- [INU 정보기술대 전용 SNS](https://github.com/ESCSNSN)
